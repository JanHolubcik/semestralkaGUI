...
namespace structures
{
    ...
    {
        LinkedList<T>* otherList = dynamic_cast<LinkedList<T>*>(&other);
        if (otherList == nullptr || size_ != otherList->size_) {
            return false;
        }
        LinkedListItem<T>* curThis = first_;
        LinkedListItem<T>* curOtherList = otherList->first_;
        while (curThis != nullptr) {
            if (curThis->accessData() != curOtherList->accessData()) {
                return false;
            }
            curThis = curThis->getNext();
            curOtherList = curOtherList->getNext();
        }
    ...
    {
        if (size_ == static_cast<int>(index)) {
            add(data);
        }
        else {
            //Utils::rangeCheckExcept(index, size_, "LinkedList<T>::insert");
            LinkedListItem<T>* newItem = new LinkedListItem<T>(data);
            if (index == 0) {
    ...
    {
        //Utils::rangeCheckExcept(index, size_, "LinkedList<T>::removeAt");
    ...
    {
        //Utils::rangeCheckExcept(index, size_, "LinkedList<T>::getItemAtIndex");
