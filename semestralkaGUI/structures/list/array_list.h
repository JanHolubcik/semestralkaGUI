...
namespace structures
{
	...
	inline bool ArrayList<T>::equals(Structure& other)
	{
		...
		if (otherList != nullptr && size_ == otherList->size_) {
			return array_->equals(*otherList->array_);
		}
		return false;
	}
	...
	{
		//Utils::rangeCheckExcept(index, this->size_, "ArrayList<T>::at");
		return (*array_)[index];
	}
	...
	{
		if (this->size_ == static_cast<int>(index)) {
			this->add(data);
		}
		else {
			//Utils::rangeCheckExcept(index, this->size_, "ArrayList<T>::insert");
	...
	{
		T result = (*this)[index];
		Array<T>::copy(*array_, static_cast<int>(index + 1), *array_, static_cast<int>(index),
			static_cast<int>(this->size_ - index - 1));
		this->size_--;
		return result;
	}
