...
namespace structures
{
	...
	Array<T>::Array(Array<T>& other) :
		vector_(new Vector(*(other.vector_))),//ocakava vektor samotny preto derefencujeme
		size_(other.size_)
	{
	}
	...
	Structure& Array<T>::assign(Structure& other)
	{
		if (this != &other) {
			...
			if (otherArray.size_ == size_) {
				//
				vector_->assign(*otherArray.vector_);// musim derefencovat potrebujem objekt nie pointer
	...
	{
		//ziskanie indexu vo vektore pomocou mapovacej funkcie
		// ziskanie byte* na danom indexe (getBytePointer)
		// pretypovanie ukazovatela z kroku 2 na T*
		// vrat to, co je pod ukazovatelom z kroku 3
		int indexV = mapFunction(index);
		byte* pointerB = vector_->getBytePointer(indexV);
		T* pointerT = reinterpret_cast<T*>(pointerB);
		return *pointerT;
	}
