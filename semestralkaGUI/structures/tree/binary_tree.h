...
namespace structures
{
	...
	{
		KWayTreeNode<T, 2>::replaceSon(leftSon, LEFT_SON);
	}

	template<typename T>
	inline void BinaryTreeNode<T>::setRightSon(BinaryTreeNode<T>* rightSon)
	{
		KWayTreeNode<T, 2>::replaceSon(rightSon, RIGHT_SON);
	}
	...
	template<typename T>
	void inline  BinaryTree<T>::InOrderTreeIterator::populatePath(TreeNode<T>* current)
	{
		if (current != nullptr) {
